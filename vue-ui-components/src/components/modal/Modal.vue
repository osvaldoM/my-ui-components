    ...
    <div class="modal-mask" v-if="props.isModalVisible">
      <div class="modal-wrapper">
        <div class="modal-container rounded-t-2xl xl:rounded-xl xl:shadow-2xl">
          <button class="w-full text-center xl:hidden"
                  @click="listeners['update:isModalVisible'](false)">
            <span class="bg-gray-200 w-8 h-1 rounded-2xl inline-block"></span>
          </button>
          <div class="modal-header">
            <slot name="header">
              <button class="modal-default-button hidden xl:block"
                      @click="listeners['update:isModalVisible'](false)">
                x
              </button>
            </slot>
...
.modal-mask {
  ...
  width: 100%;
  height: 100%;
  background-color: rgba(63, 60, 58, 78);
  display: table;
